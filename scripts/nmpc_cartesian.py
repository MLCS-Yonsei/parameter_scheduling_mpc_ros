    ...
    if warm_start==1:
      ...
      for k in range(0, self.N):
        ...
        if r2 < EPS:
          ...
          cr = -np.cos(x_warm[2]) * x_warm[0] - np.sin(x_warm[2]) * x_warm[1]
          sr = np.sin(x_warm[2]) * x_warm[0] - np.cos(x_warm[2]) * x_warm[1]
          if cr >=0:
            omega = self.omega_max * np.arctan2(sr, cr) * 2 / np.pi
          else:
            omega = -self.omega_max * np.arctan2(sr, -cr) * 2 / np.pi
