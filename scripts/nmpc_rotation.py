...
class NmpcRotation:
  ...
  def __init__(
    ...
    rhs = vertcat(v*cos(psi), v*sin(psi), omega) # system r.h.s

    self.f = Function('f', [states, v, omega], [rhs]) # nonlinear mapping function f(x,u), system equation 
    U = Variables('U') # Decision variables (controls), optimal control input as determined by optimizer 
    P = SX.sym('P', self.n_states + self.n_controls + 2) # parameters (which include the initial and the reference/final desired state of the robot, and the cost function parameters)
    ...
    for k in range(0, self.N):
      ...
      if ode=='RK4':
        st_prediction = RK4(self.f, T, st, vel[0], vel[1])
      if ode=='RK2':
        st_prediction = RK2(self.f, T, st, vel[0], vel[1])
      else:
        st_prediction = st + self.f(st, vel[0], vel[1]) * T
  ...
  def __dx(x, v, omega):
    return np.array(
      [v*np.cos(x[2]), v*np.sin(x[2]), omega]
    )
  ...
  def solve(self, state, theta, u_prev=None, return_trajectory=False, warm_start=0):
    ...
    x_init = [
      [state[0] * np.cos(np.pi + state[1] - state[2])],
      [state[0] * np.sin(np.pi + state[1] - state[2])],
      [-state[2]]
    ]
    if u_prev is None:
      self.args['p'] = vertcat(
    ...
    if warm_start==1:
      ...
      u_warm = u_prev
      for k in range(0, self.N):
        r2 = x_warm[0] ** 2 + x_warm[1] ** 2
        v = 0
        if x_warm[2] == 0.:
          omega = 0
        else:
          omega = -self.omega_max * x_warm[2] / np.pi
        scale = max(np.abs(omega / self.omega_max), 1)
        omega /= scale
        if self.dw_max < np.inf:
          if u_warm is None:
            u_warm = np.array(
              [
                (-omega*self.wheel_separation/2)/self.wheel_rad,
                (omega*self.wheel_separation/2)/self.wheel_rad
              ]
            )
...
if __name__ == '__main__':
  ...
  tic = time.time()
  X, U = nmpc.solve(
    [0.5, np.pi/4, -np.pi/4], # initial state
    1,             # cost function weight
    u_prev=[0, 0],
    warm_start=False,
